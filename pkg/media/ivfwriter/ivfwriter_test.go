...
func TestIVFWriter_Basic(t *testing.T) {
	...
	assert.Equal(uint64(0), writer.frameCount, "Writer's packet frameCount should initialize 0")
	err = writer.Close()
...
func TestIVFWriter_VP8(t *testing.T) {
	...
	assert.Equal(uint64(0), addPacketTestCase[0].writer.frameCount, "Writer's packet frameCount should remain 0")
	...
	assert.Equal(uint64(1), addPacketTestCase[1].writer.frameCount, "Writer's packet frameCount should now be 1")
	assert.Equal(nil, addPacketTestCase[1].writer.WriteRTP(midPartPacket), "Write packet failed") // add a mid partition packet
	assert.Equal(uint64(1), addPacketTestCase[1].writer.frameCount, "Writer's packet frameCount should remain 1")
	assert.Equal(nil, addPacketTestCase[1].writer.WriteRTP(validPacket), "Write packet failed") // add a valid packet
	assert.Equal(uint64(2), addPacketTestCase[1].writer.frameCount, "Writer's packet frameCount should now be 2")
